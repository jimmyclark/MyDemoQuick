...
function Path:ctor(id, state, map)
    ...
    self.m_valid = #self.m_points > 0;
end

function Path:getPoint(index)
    local pt = self.m_points[index];
    return pt[1], pt[2];
end
...
function Path:checkPointAtPoint(x, y, checkDist)
    ...
    local minDistIndex;
    for index, pt in ipairs(self.m_points) do
...
function Path:checkPointAtSegment(x, y, checkDist)
    local minDist = 999999;
    local minDistIndex;
    for index = 1, #self.m_points - 1 do
        local ax, ay = unpack(self.m_points[index]);
        local bx, by = unpack(self.m_points[index + 1]);
...
function Path:vardump()
    ...
    state.points = clone(self.m_points);

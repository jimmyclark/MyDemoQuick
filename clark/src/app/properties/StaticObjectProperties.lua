...
----------------------------------------
function StaticObjectProperties.getAllIds()
    local keys = table.keys(defines);
    table.sort(keys);
    return keys;
end

function StaticObjectProperties.get(defineId)
    assert(defines[defineId], string.format("StaticObjectProperties.get() - invalid defineId %s", tostring(defineId)));
    return clone(defines[defineId]);
end

function StaticObjectProperties.isExists(defineId)

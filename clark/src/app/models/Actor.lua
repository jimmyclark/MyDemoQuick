...
function Actor:ctor(properties, events, callbacks)
    ...
    self.m_state = self:getComponent("components.behavior.StateMachine");
    ...
    local defaultCallbacks = {
        ...
        onfire        = handler(self, self.onFire),
        onready       = handler(self, self.onReady),
        onfreeze      = handler(self, self.onFreeze),
        onthaw        = handler(self, self.onThaw),
        onkill        = handler(self, self.onKill),
        onrelive      = handler(self, self.onRelive),
        onleavefiring = handler(self, self.onLeaveFiring),
    };
    -- 如果继承类提供了其他回调，则合并
    table.merge(defaultCallbacks, checktable(callbacks));

    self.m_state:setupState({
        events = defaultEvents,
        callbacks = defaultCallbacks
    });

    self.m_state:doEvent("start"); -- 启动状态机
end
...
function Actor:getState()
    return self.m_state:getState();
end

function Actor:canFire()
    return self.m_state:canDoEvent("fire");
end

function Actor:isDead()
    return self.m_state:getState() == "dead";
end

function Actor:isFrozen()
    return self.m_state:getState() == "frozen";
end
...
-- 开火
function Actor:fire()
    self.m_state:doEvent("fire");
    self.m_state:doEvent("ready", Actor.FIRE_COOLDOWN);
end
...
function Actor:onChangeState(event)
